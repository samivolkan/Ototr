...
class InspectionProgressScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalItems = order.modules.fold<int>(0, (sum, module) => sum + module.checklistCount);
    final completedItems = order.modules.fold<int>(0, (sum, module) => sum + module.completedCount);
        ...
        children: [
          OtotrCard(
            child: Column(
              ...
              children: [
                Text('%${(progress * 100).round()} tamamlandı', style: Theme.of(context).textTheme.titleLarge),
                const SizedBox(height: AppSizes.md),
                LinearProgressIndicator(value: progress, minHeight: 10, color: AppColors.red),
                const SizedBox(height: AppSizes.md),
          ...
          const OtotrAlertCard(
            ...
            message: 'Sol yan, kilometre göstergesi ve VIN etiketi fotoğrafları tamamlanmadan rapor onayına geçilmemeli.',
          ),
          const OtotrSectionTitle(title: 'Aksiyonlar'),
          OtotrPrimaryButton(
            label: 'Modül Kontrollerine Devam Et',
            icon: Icons.fact_check_outlined,
            onPressed: () => Navigator.pushNamed(context, AppRoutes.inspectionModules),
          ),
          const SizedBox(height: AppSizes.sm),
          OtotrSecondaryButton(
            label: 'Fotoğraf Kanıtına Git',
            icon: Icons.photo_camera_outlined,
            onPressed: () => Navigator.pushNamed(context, AppRoutes.photoEvidence),
          ),
          const SizedBox(height: AppSizes.sm),
          OtotrSecondaryButton(
            label: 'Modülü Tamamlandı İşaretle',
            icon: Icons.done_outline,
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Modül tamamlandı placeholder. Sonradan Firebase sync.'))),
          ),
        ],
      ),
    );
